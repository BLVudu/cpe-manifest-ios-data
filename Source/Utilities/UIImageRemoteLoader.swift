...
public struct UIImageRemoteLoader {
    ...
    public static func loadImage(url: NSURL, completion: ((image: UIImage?) -> Void)?) -> NSURLSessionDataTask? {
        ...
        let task = NSURLSession(configuration: sessionConfiguration).dataTaskWithRequest(NSURLRequest(URL: url)) { (data, response, error) -> Void in
            if let completion = completion {
                completion(image: (data != nil ? UIImage(data: data!, scale: UIScreen.mainScreen().scale) : nil))
            }
        }
